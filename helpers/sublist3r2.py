...
def parse_args():
    ...
    parser.add_argument('-p', '--ports',
                        help='Check/Filter subdomain results for open ports (provide comma separated ports')
    parser.add_argument('-v', '--verbose', help='Enable verbose mode and display results in realtime', nargs='?',
                        default=False)
    parser.add_argument('-t', '--threads', help='Number of threads to use for aiodnsbrute bruteforce', type=int,
                        default=7000)
...
class Virustotal(enumratorBaseThreaded):
    ...
    def enumerate(self):
        if self.apikey:
            ...
            self.print_(
                R + "[!] set VT_APIKEY to your virus total API key using:   export VT_APIKEY=Your_VT_API_KEY_VALUE" + W)

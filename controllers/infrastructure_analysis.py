...
add_to_db = Queue(name='infrastructureAnalysis_db_queue', connection=Redis(host=os.environ.get('REDIS_HOST')))
...
class InfrastructureAnalysis(Resource):
    ...
    def post():
        ...
        list_scans = {}
        val = args['value']
        db.infrastructureAnalysis.create_index('value')
        # see if we have an existing scan for given value and pull the latest
        search = db.infrastructureAnalysis.find_one({'value': val}, sort=[('_id', pymongo.DESCENDING)])
        ...
        if search is None or force is True:
            ...
            if validators.domain(val) or validators.ip_address.ipv4(val):
                ...
                item = db.infrastructureAnalysis.insert_one(
                    {'ip': ip, 'value': val,
        ...
        add_to_db.enqueue(queue_to_db.infrastructure_analysis_db_addition, message, retry=Retry(max=3, interval=[10, 30, 60]))

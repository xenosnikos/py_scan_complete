...
# add_to_db = Queue(name='sslConfiguration_db_queue', connection=Redis(host=os.environ.get('REDIS_HOST')))
...
class SSLConfiguration(Resource):
    ...
    def post():
        ...
        list_scans = {}
        val = args['value']
        db.sslConfiguration.create_index('value')
        # see if we have an existing scan for given value and pull the latest
        search = db.sslConfiguration.find_one({'value': val}, sort=[('_id', pymongo.DESCENDING)])
        ...
        if search is None or force is True:
            ...
            if validators.domain(val) or validators.ip_address.ipv4(val):
                ...
                item = db.sslConfiguration.insert_one(
                    {'ip': ip, 'value': val,
                ...
                resp = session.get(f"https://api.threatintelligenceplatform.com/v1/sslConfiguration?domainName="
        ...
        #            'data': list_scans}
        #
        # add_to_db.enqueue(queue_to_db.ssl_configuration_db_addition, message, retry=Retry(max=3, interval=[10, 30, 60]))

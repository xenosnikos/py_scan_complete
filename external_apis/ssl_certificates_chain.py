...
class SSLCertificatesChain(Resource):
    ...
    def post():
        ...
        list_scans = {}
        val = args['value']
        db.sslCertificatesChain.create_index('value')
        # see if we have an existing scan for given value and pull the latest
        search = db.sslCertificatesChain.find_one({'value': val}, sort=[('_id', pymongo.DESCENDING)])
        ...
        if search is None or force is True:
            ...
            if validators.domain(val) or validators.ip_address.ipv4(val):
                ...
                item = db.sslCertificatesChain.insert_one(
                    {'ip': ip, 'value': val,
